...
display = Display(spiTFT, dc=Pin(TFT_DC_PIN), cs=Pin(TFT_CS_PIN), rst=Pin(TFT_RST_PIN), width=320, height=240, rotation=270)
...
def startup_screen():
    palette = create_palette(color565(255, 255, 255), 0, True)
    sleep_icu, w, h = load_pbm('sleepICU.pbm')
    placeholder = bytearray(w * h * 2)
    placeholder_fb = FrameBuffer(placeholder, w, h, RGB565)
    placeholder_fb.blit(sleep_icu, 0, 0, -1, palette)
    display.draw_sprite(placeholder, 80, 50, w, h)
            ...
            with open(csv_filename, "a") as csvfile:
                ...
                while utime.time() < end_time:
                    ...
                    try:
                        ...
                        elif len(data) == 12:  # Normal data
                            temp, light, sound = unpack('!fff', data)
